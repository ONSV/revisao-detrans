...
tbl_red <- "#df555d"
map_red <- "#e47076"
col_func <- colorRampPalette(colors = c(map_red, map_blue))
```
...
A @tbl-condutores mostra o desempenho médio dos DETRANs na disponibilização de dados de condutores habilitados. Novamente, a periodicidade e a interatividade são os critérios com maior e menor performance geral respectivamente:
...
Devido ao fato da avaliação dos CFCs ser feita a partir de um único critério de avaliação, como indica a @tbl-cri-cfcs, as notas para cada estado estão ilustradas na @tbl-cfcs em valores qualitativos:
...
df_revisoes |>
  ...
  ggplot() +
    ...
    geom_point(aes(ano_2020, uf, color = "2020"), size = 2) +
    geom_point(aes(ano_2024, uf, color = "2024"), size = 2) +
    scale_color_manual(values = c(onsv_palette$red, onsv_palette$blue)) +
    labs(x = "Pontuação", y = NULL, color = "Ano") +
    theme_minimal() +
...
df_revisoes |>
  ...
  mutate(
    ...
    name = case_match(
      ...
      "ano_2020" ~ "2020",
      "ano_2024" ~ "2024",
      "difer" ~ "Diferença percentual"
    )
  ) |>
  ggplot(aes(value, uf, fill = fill)) +
  geom_bar(stat = "identity") +
  facet_wrap(vars(name), scales = "free_x") +
  scale_fill_manual(values = c(onsv_palette$red, onsv_palette$blue, onsv_palette$green)) +
  theme_minimal() +
