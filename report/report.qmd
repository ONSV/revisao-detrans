...
O mapa das notas atribuídas demonstrado na @fig-map-condutores permite observar uma leve diferença na disponibilidade de informações de condutores em comparação à frota veicular.
...
A @fig-map-educ demonstra como a maioria dos estados possui disponibilidade adequada de conteúdos didáticos e atividades educativas.
...
É possível observar os estados que mais evoluíram em relação a sua pontuação anterior observando as variações percentuais entre 2024 e 2020, como aponta a @fig-var-percent .
...
comparison_map |>
  ...
  geom_sf(aes(fill = nota_final)) +
  geom_sf_text(
    aes(label = round(nota_final, 1)),
    nudge_x = comparison_map$nudge_x_text,
    nudge_y = comparison_map$nudge_y_text,
    color = ifelse(comparison_map$nota_final >= 7, "red", "black"),
    size = 2
  ) +
  facet_wrap(vars(ano)) +
  theme_minimal() +
...
consulta_pre |> 
  ...
  data_color(nota_final:var, method = "numeric", palette = "RdBu")
```
