...
make_reactable <- function(data, ...) {
  ...
  reactable(
    ...
    theme = reactableTheme(
      style = list(fontFamily = font_families, fontSize = "0.60rem"),
...
consulta_pre |>
  ...
  make_reactable(columns = list(
    Frota = colDef(style = bi_bg_func),
    Condutores = colDef(style = bi_bg_func),
    `Infrações` = colDef(style = bi_bg_func),
    Sinistros = colDef(style = bi_bg_func),
    Atendimento = colDef(style = bi_bg_func),
    `Educação` = colDef(style = bi_bg_func),
    Cfcs = colDef(style = bi_bg_func)
  ))
```
...
df_revisoes |>
  ...
  theme_bw() +
  facet_grid(vars(regiao), scales = "free", space = "free_y", switch = "y") +
