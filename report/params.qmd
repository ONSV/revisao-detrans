...
O presente documento tem por objetivo apresentar uma avaliação do nível de informações disponibilizadas no portal (sítio eletrônics) do DETRAN (Departamento Estadual de Trânsito) `r if(params$state %in% c("BA","PB")) {paste("da", get_state_name(params$state))} else {paste("de", get_state_name(params$state))}`.
...
A pontuação final de 2024 para o estado em destaque foi computada por meio da média destas notas, atingindo um valor de **`r round(filter(preliminar, ano == 2024)$nota_final, 2)`**, o que representa `r if(dif == 0) {paste("uma", sentido)} else {paste("um", sentido)}` em relação a 2020.
...
```
Em síntese, a`r if(max_values_len > 1) {paste0("s")}` categoria`r if(max_values_len > 1) {paste0("s")}` de melhor desempenho para o DETRAN `r if(params$state %in% c("BA","PB")) {paste("da", get_state_name(params$state))} else {paste("de", get_state_name(params$state))}` `r if(max_values_len <= 1) {paste("foi a de")} else {paste("foram as de")}` `r knitr::combine_words(str_to_title_func(replace_func(max_values$name)), and = " e ", oxford_comma = F)`, com uma pontuação de .
