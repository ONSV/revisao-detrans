...
A pontuação final de 2024 para o estado em destaque é de `r round(filter(preliminar, ano == 2024)$nota_final, 2)`, o que representa `r if(dif == 0) {paste("uma", sentido)} else {paste("um", sentido)}` em relação à 2020.

O resultado final das consultas, efetuadas em 2020 e 2024, sobre as categorias e critérios considerados para a avaliação da disponibilidade de dados no portal do DETRAN-`r params$state` está disponível na @tbl-final. A nota final é 
