...
O presente documento tem por objetivo apresentar uma avaliação do nível de informações disponibilizadas no portal (sítio eletrônico) do DETRAN (Departamento Estadual de Trânsito) `r if(params$state %in% c("BA","PB")) {paste("da", get_state_name(params$state))} else {paste("de", get_state_name(params$state))}`.
...
Nesse sentido, este documento busca apresentar uma análise da situação deste DETRAN, permitindo obter um panorama de como esta instituições disponibiliza informações de interesse social. Além da introdução, o documento compõe-se das seguintes seções: @sec-metodos, com as consultas preliminares, pontuação e critérios estabelecidos; e @sec-resultados, com a pontuação geral em cada categoria analisada, com a evolução das notas resultantes ao longo dos anos e com os exemplos de boas práticas encontradas nos sítios eletrônicos.
...
A fim de analisar quantitativamente os índices atribuídos para cada tipo de informação consultada, os dados de 2020 e 2024 são reunidos e a nova versão versão do estudo é comparada à anterior. A nota resultante para cada categoria de dado considerada é calculada por meio da média das pontuações de seus critérios, como aponta @tbl-media.  
...
```
Em síntese, o presente trabalho pretende avaliar de forma quantitativa a performance do portal virtual do departamento consultado em disponibilizar ao público informações estatísticas pertinentes à segurança viária do respectivo estado. Desta maneira, a`r if(max_values_len > 1) {paste0("s")}` categoria`r if(max_values_len > 1) {paste0("s")}` de melhor desempenho para o DETRAN `r if(params$state %in% c("BA","PB")) {paste("da", get_state_name(params$state))} else {paste("de", get_state_name(params$state))}` `r if(max_values_len <= 1) {paste("foi a de")} else {paste("foram as de")}` `r knitr::combine_words(str_to_title_func(replace_func(max_values$name)), and = " e ", oxford_comma = F)`, com nota igual a `r format(unique(max_values$value), decimal.mark = ",")`. 

Em contrapartida, a`r if(min_values_len > 1) {paste0("s")}` categoria`r if(min_values_len > 1) {paste0("s")}` de pior desempenho `r if(min_values_len <= 1) {paste("foi a de")} else {paste("foram as de")}` `r knitr::combine_words(str_to_title_func(replace_func(min_values$name)), and = " e ", oxford_comma = F)`, com nota `r format(unique(min_values$value), decimal.mark = ",")`. Estes resultados podem auxiliar na orientação de melhorias para o sítio eletrônico, visando aumentar a qualidade e disponibilidade dos dados fornecidos em conformidade às suas atribuições. 
